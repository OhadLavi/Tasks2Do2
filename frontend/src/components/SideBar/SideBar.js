...
import { FaSignOutAlt, FaBars, FaPlus, FaTasks, FaCalendarPlus, FaCalendarCheck, FaClock, FaSquareFull } from 'react-icons/fa';
...
const SidebarPage = () => {
    ...
    const pages = [
        ...
        { name: 'Upcoming', link: '/tasks/upcomingTasks', icon: <MdDoubleArrow />, suffix: '' },
        { name: 'Add New Task', link: '/addTask', icon: <FaCalendarPlus />, suffix: '' },
        { name: 'Completed', link: '/tasks/completedTasks', icon: <FaCalendarCheck />, suffix: '' },
        { name: 'Expired', link: '/tasks/expiredTasks', icon: <FaClock />, suffix: '' },
    ...
    return (
        ...
        >
            ...
            >
                ...
                <SidebarHeader>
                    <Menu iconShape="circle">
                        {collapsed ? (
                            <MenuItem icon={<FaBars />} onClick={handleCollapsedChange}></MenuItem>
                        ) : (
                ...
                <SidebarContent>
                    <Menu iconShape="circle">
                        {pages.map((page, index) => (
                            <MenuItem key={index} icon={page.icon} suffix={<span className="badge red">{page.suffix}</span>}>
                                {page.name}
                ...
                {/* Footer */}
                <SidebarFooter>
                    <div className="sidebar-btn-wrapper d-flex justify-content-center" style={{ padding: '16px' }}>
                        <Link
            ...
            <div className={`${display ? "d-inline-block ms-2" : "display"}`} onClick={breakPoint}>
                <FaBars />
            </div>
        </Box>
    );
